...
import com.wits.grofastsupplier.Notification.NotificationInterface;
import com.wits.grofastsupplier.Notification.NotificationModel;
import com.wits.grofastsupplier.Notification.NotificationResponse;
...
public class Notification_setting extends AppCompatActivity {
    ...
    private void notificationFetch() {
        Call<NotificationResponse> call1 = Retrofirinstance.getClient(supplierActivitySession.getToken()).create(NotificationInterface.class).notificationFetch();
    ...
    private void sendNotificationUpdate(boolean isEnableAll, int enable_all) {
        Call<NotificationResponse> call = Retrofirinstance.getClient(supplierActivitySession.getToken()).create(NotificationInterface.class).notificationsettingEnable(enable_all, push.isChecked() ? 1 : 0, newsletter.isChecked() ? 1 : 0, promoemail.isChecked() ? 1 : 0, promopush.isChecked() ? 1 : 0, socialemail.isChecked() ? 1 : 0, socialpush.isChecked() ? 1 : 0);
...
//                    Toast.makeText(NotificationSetting.this, "" + notificationResponse.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private void sendNotificationUpdate(boolean isEnableAll, String setting, int value) {
        ...
        Call<NotificationResponse> call = Retrofirinstance.getClient(supplierActivitySession.getToken()).create(NotificationInterface.class).notificationsetting(push_notification, newsletter_email, promo_offer_email, promo_offer_push, social_notification_email, social_notification_push);
        call.enqueue(new Callback<NotificationResponse>() {
            @Override
            public void onResponse(Call<NotificationResponse> call, Response<NotificationResponse> response) {
                if (response.isSuccessful()) {
